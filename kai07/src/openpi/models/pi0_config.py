...
@dataclasses.dataclass(frozen=True)
class Pi07Config(Pi0Config):
    """Config for pi0.7 model architecture.

    Key differences from pi0.5:
    - 448x448 image resolution (vs 224x224)
    - 860M action expert (vs 300M)
    - MEM history encoder with multi-frame temporal compression
    - Linear state projection (vs discrete tokenized state)
    - Block-causal attention mask
    - Supports subgoal images
    """

    pi05: bool = True  # pi0.7 builds on pi0.5 backbone
    pi07: bool = True

    # Image resolution (pi0.7 uses 448x448)
    image_resolution: int = 448

    # Action expert (860M for pi0.7)
    action_expert_variant: _gemma.Variant = "gemma_860m"

    # MEM history encoder settings
    max_history_frames: int = 6  # max history frames (1s stride at 30fps)
    history_stride: int = 30  # frames between history observations

    # Subgoal settings
    max_subgoal_images: int = 3  # one per camera view

    # State projection (linear instead of discrete tokens)
    discrete_state_input: bool = False

    # Inference latency simulation (train-time observation delay)
    max_obs_delay: int = 12  # max frames to delay observation
    ...
    def __post_init__(self):
        if self.max_token_len is None:
            # pi0.7 needs more tokens for richer prompts
            object.__setattr__(self, "max_token_len", 256)

    @property
    @override
    def model_type(self) -> _model.ModelType:
        return _model.ModelType.PI07
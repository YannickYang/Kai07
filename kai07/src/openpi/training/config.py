...
class DataConfig:
    ...
    # NEW: optional list of episode ids to load/filter
...
class ModelTransformFactory(GroupFactory):
    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> _transforms.Group:
        match model_config.model_type:
            ...
            case _model.ModelType.PI07:
                # pi0.7: 448x448 resolution, richer tokenization
                assert isinstance(model_config, pi0_config.Pi07Config)
                return _transforms.Group(
                    inputs=[
                        # InjectPi07Prompt is added at the data config level (needs annotation data)
                        _transforms.ResizeImages(model_config.image_resolution, model_config.image_resolution),
                        _transforms.TokenizePrompt(
                            _tokenizer.PaligemmaTokenizer(model_config.max_token_len),
                            discrete_state_input=False,  # pi0.7 uses linear state projection
                        ),
                        _transforms.PadStatesAndActions(model_config.action_dim),
                    ],
                )
...
@dataclasses.dataclass(frozen=True)
class LerobotPi07DataConfig(DataConfigFactory):
    """Data config for pi0.7 training with subtask annotations and episode metadata.

    Extends the Agilex data pipeline with:
    - Subtask annotation loading (per-frame subtask text)
    - Episode metadata loading (speed, quality, mistake)
    - pi0.7-style prompt construction with dropout
    ...
    # Path to subtask annotations JSON (from annotate_subtasks.py)
    subtask_annotations_path: str | None = None
    # Path to episode metadata JSON (from annotate_metadata.py)
    episode_metadata_path: str | None = None

    # Control mode for prompt (e.g., "joint", "cartesian")
    control_mode: str = "joint"

    # If true, will convert joint dimensions to deltas
                ...
                _transforms.RepackTransform(
                    {
                        "images": {
                            ...
                            "hand_left": "observation.images.left",
                            "hand_right": "observation.images.right",
                        },
                        "state": "observation.state",
                        "actions": "action",
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        if self.subtask_annotations_path:
            with open(self.subtask_annotations_path, "r") as f:
                subtask_annotations = json.load(f)

        # Load episode metadata
        episode_metadata = None
        if self.episode_metadata_path:
            with open(self.episode_metadata_path, "r") as f:
                episode_metadata = json.load(f)

        # Create pi0.7 prompt transform (injected before model transforms)
        pi07_prompt_transform = _transforms.InjectPi07Prompt(
            subtask_annotations=subtask_annotations,
            episode_metadata=episode_metadata,
            control_mode=self.control_mode,
        )

        # Subgoal image loader (25% of samples include subgoal images)
        subgoal_dir = None
        if self.repo_id:
            candidate = os.path.join(self.repo_id, "subgoal_images")
            if os.path.isdir(candidate):
                subgoal_dir = candidate
        subgoal_loader = _transforms.LoadSubgoalImages(
        ...
        # Model transforms (pi0.7 variant)
        model_transforms = ModelTransformFactory(default_prompt=self.default_prompt)(model_config)
...
_CONFIGS = [
    ...
    #
    TrainConfig(
        name="pi07_fold_clothes",
        model=pi0_config.Pi07Config(
            action_dim=14,
            action_horizon=50,
        ),
        data=LerobotPi07DataConfig(
            repo_id="/home/woan/kai0_train/data/fold_clothes_20260428_1531_merge266",
            default_prompt="Flatten and fold the cloth",
            subtask_annotations_path="/home/woan/kai0_train/data/fold_clothes_20260428_1531_merge266/subtask_annotations.json",
            episode_metadata_path="/home/woan/kai0_train/data/fold_clothes_20260428_1531_merge266/episode_metadata.json",
            control_mode="joint",
            use_delta_joint_actions=False,
        ),
        pytorch_weight_path="/home/woan/kai0_train/weights/pi05_base",
        num_train_steps=100_000,
        keep_period=10_000,
        num_workers=8,
        batch_size=32,
    ...
    TrainConfig(
        ...
        data = LerobotAgilexDataConfig(
            # # data181 / dagger100
            # repo_id="/mnt/data/data/raw_data/data_piper_aloha/crumpled_clothes/actions_designed_v1_dagger/data181_dagger100_merge_202603301846",
            # data181 / dagger204
            repo_id="/mnt/data/yzj_data/dataset/fold_clothes_20260428_1531_merge266",
            default_prompt="Flatten and fold the cloth",
            # repo_id="<path_to_repo_root>/data/FlattenFold/base",
            # default_prompt="Flatten and fold the cloth.",
            use_delta_joint_actions=False,

            # override repack mapping to match this dataset
            repack_transforms=_transforms.Group(
                inputs=[
                    _transforms.RepackTransform(
                        {
                            "images": {
                                # model expects these canonical names:
                                "top_head": "observation.images.head",
                                "hand_left": "observation.images.left",
                                "hand_right": "observation.images.right",
                            },
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("/mnt/data/hzy_data/home_cache/pi05_base/params"),
        num_train_steps=100_000,
        keep_period=5000,
        num_workers=8,
        batch_size=32,
    ...
    TrainConfig(
        name="pi05_flatten_fold_normal_1",
        model=pi0_config.Pi0Config(pi05=True),
        data = LerobotAgilexDataConfig(
            repo_id="/mnt/data/data/raw_data/data_piper_aloha/crumpled_clothes/actions_designed_v1_dagger/dagger204_merge_202603312111",
            default_prompt="Flatten and fold the cloth",
            # repo_id="<path_to_repo_root>/data/FlattenFold/base",
            # default_prompt="Flatten and fold the cloth.",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        ...
        pytorch_weight_path="/mnt/data/hzy_data/home_cache/pi05_base/params",
        num_train_steps=100_000,
        keep_period=10000,
        save_interval=10000,
        num_workers=8,
    ...
    TrainConfig(
        name="ADVANTAGE_TORCH_KAI0_FLATTEN_FOLD_base_split_175",
        advantage_estimator=True,
        model=pi0_config.AdvantageEstimatorConfig(
            pi05=True,
            loss_value_weight=1.,
        ...
        data=LerobotAgilexDataConfig(
            repo_id = "/mnt/data/hzy_data/project/kai0_ws/data/test_data/base_split_175_edit",
            assets=AssetsConfig(
                assets_dir="/mnt/data/hzy_data/project/kai0_ws/data/test_data/",
                asset_id="base_split_175_edit",
            ),
            default_prompt="Flatten and fold the cloth",
            # * why removing "prompt" here will lead to an error in transforms.py
            repack_transforms=_transforms.Group(
        ...
        pytorch_weight_path="/mnt/data/hzy_data/home_cache/huggingface/hub/models--lerobot--pi05_base/snapshots/9e55186ad36e66b95cda57bc47818d9e6237ae30",
        num_train_steps=100_000,
        keep_period=10000,
        save_interval=10000,
        num_workers=8,
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(pi05=True),
        data = LerobotAgilexDataConfig(
            repo_id="/mnt/data/hzy_data/project/kai0_ws/data/die_yi_fu/fold_clothes_20260212_1950_old_stage_label_advantage",
            default_prompt="Flatten and fold the cloth",
            # repo_id="<path_to_repo_root>/data/FlattenFold/advantage",
            # default_prompt="Flatten and fold the cloth.",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_flatten_fold_awbc_1",
        model=pi0_config.Pi0Config(pi05=True),
        data = LerobotAgilexDataConfig(
            repo_id="/mnt/data/hzy_data/project/kai0_ws/data/die_yi_fu/fold_clothes_20260212_1950_old_stage_label_relative_advantage",
            default_prompt="Flatten and fold the cloth",
            # repo_id="<path_to_repo_root>/data/FlattenFold/advantage",
            # default_prompt="Flatten and fold the cloth.",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_flatten_fold_awbc_base_split_175_adv_181_data",
        model=pi0_config.Pi0Config(pi05=True),
        data = LerobotAgilexDataConfig(
            repo_id="/mnt/data/hzy_data/project/kai0_ws/data/die_yi_fu/fold_clothes_20260212_1950_old_base_split_175_advantage",
            default_prompt="Flatten and fold the cloth",
            use_delta_joint_actions=False,

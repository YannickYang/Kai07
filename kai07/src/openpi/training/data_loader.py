...
class ObsDelayDataset(Dataset):
    """Simulates inference latency by using an earlier frame's observation.

    For each sample at global index `idx`, randomly picks delay d ∈ [0, max_delay].
    Loads observation (images + state) from frame idx-d, but keeps the action
    target from frame idx. Respects episode boundaries (won't cross episodes).

    This trains the model to be robust to the ~0.3-0.4s planning latency
    that exists during real-time inference (pi0.7 paper Section 4.3).
    """

    def __init__(self, dataset, max_delay: int = 12):
        self._dataset = dataset
        self._max_delay = max_delay
        # Build episode boundary lookup: global_index → episode start index
        self._ep_starts = {}
        if hasattr(dataset, 'episode_data_index'):
            edi = dataset.episode_data_index
            from_indices = edi['from']
            to_indices = edi['to']
            for ep_i in range(len(from_indices)):
                fr = int(from_indices[ep_i])
                to = int(to_indices[ep_i])
                for gi in range(fr, to):
                    self._ep_starts[gi] = fr
    ...
    def __getitem__(self, index: SupportsIndex) -> dict:
        ...
        # Pick random delay
        d = _random.randint(0, self._max_delay)
        if d == 0:
            current["obs_delay"] = np.int32(0)
            return current

        # Clamp to episode start
        ep_start = self._ep_starts.get(idx, 0)
        delayed_idx = max(ep_start, idx - d)
        actual_delay = idx - delayed_idx

        if actual_delay == 0:
            current["obs_delay"] = np.int32(0)
            return current

        # Load the delayed observation
        delayed = self._dataset[delayed_idx]

        # Replace observation keys (images + state), keep action + metadata from current
        obs_keys = [k for k in delayed.keys() if k.startswith("observation.")]
        for k in obs_keys:
            current[k] = delayed[k]

        current["obs_delay"] = np.int32(actual_delay)
        return current
...
def create_torch_dataset(
    ...
    # Pi0.7: wrap with observation delay simulation (0-12 frames)
    if getattr(model_config, "pi07", False):
        max_delay = getattr(model_config, "max_obs_delay", 12)
        dataset = ObsDelayDataset(dataset, max_delay=max_delay)

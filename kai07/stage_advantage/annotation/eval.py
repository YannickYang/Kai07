...
MODELS_CONFIG_MAP = {
    'Task-A': {
        'PI06': {
            'name': 'PI06',
            'config_name': 'ADVANTAGE_TORCH_KAI0_FLATTEN_FOLD_base_split_175',
            'ckpt_dir': 'experiment/ADVANTAGE_TORCH_KAI0_FLATTEN_FOLD_base_split_175/run1',
            'ckpt_steps': 100000
        },
        'KAI0': {
            'name': 'KAI0',
            'config_name': 'ADVANTAGE_TORCH_KAI0_FLATTEN_FOLD_base_split_175',
            'ckpt_dir': '/mnt/data/hzy_data/project/kai0_ws/kai0_git/checkpoints/ADVANTAGE_TORCH_KAI0_FLATTEN_FOLD_base_split_175/base_split_175_run_202603171643',
            'ckpt_steps': 100
        },
    },
    'Flatten-Fold': {
...
def parse_args():
    ...
    parser.add_argument('model_type', type=str, choices=['Task-A', 'Flatten-Fold', 'demo_A', 'demo_B'],
...
def main():
    ...
    for model_cfg in models_config:
        ...
        for i in tqdm(range(dataset_metadata.total_episodes), desc="Evaluating episodes"):
            ...
            # )
            print("video_path template:", dataset_metadata.video_path)
            print("chunks_size:", dataset_metadata.chunks_size)
            top_video = repo_id / dataset_metadata.video_path.format(
                episode_chunk=i // dataset_metadata.chunks_size, episode_index=i, video_key='observation.images.head'
            )
            print("video_path:", top_video)
            left_video = repo_id / dataset_metadata.video_path.format(
                episode_chunk=i // dataset_metadata.chunks_size, episode_index=i, video_key='observation.images.left'
            )
            right_video = repo_id / dataset_metadata.video_path.format(
                episode_chunk=i // dataset_metadata.chunks_size, episode_index=i, video_key='observation.images.right'
            )

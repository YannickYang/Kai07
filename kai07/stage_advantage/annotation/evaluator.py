...
class SimpleValueEvaluator:
    ...
    def _load_model(self):
        ...
        use_adv = bool(getattr(self.config, "advantage_estimator", False))
        if use_adv:
            self.model = AdvantageEstimator(new_model).to(self.device)
        else:
    ...
    def _load_video_frames(self, video_path: str, frame_interval: int = 1) -> List[np.ndarray]:
        """Load frames using PyAV (better codec support including AV1)."""
        if not os.path.exists(video_path):
            raise FileNotFoundError(f"Video file not found: {video_path}")

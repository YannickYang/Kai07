...
class FSDPCheckpoint:
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            _dev = f"cuda:{dist.get_rank()}" if dist.is_initialized() else "cpu"
            file_size = os.path.getsize(model_state_dict_path) / (1024**3)
            logger.info(f"Loading checkpoint from {resume_from} ({file_size:.1f}GB) to {_dev}...")
            import time as _t; _t0 = _t.time()
            model_state_dict = load_file(model_state_dict_path, device=_dev)
            logger.info(f"  load_file done in {_t.time()-_t0:.1f}s")
            model_state_dict.pop('latent_pos_embed.pos_embed')
            model_state_dict.pop('vit_pos_embed.pos_embed')
            msg = model.load_state_dict(model_state_dict, strict=False)
            logger.info(f"  load_state_dict done in {_t.time()-_t0:.1f}s")
            logger.info(msg)
            ...
            if ema_model is not None:
                ...
                logger.info(f"Loading ema checkpoint...")
                _t1 = _t.time()
                ema_state_dict = load_file(ema_state_dict_path, device=_dev)
                logger.info(f"  ema load_file done in {_t.time()-_t1:.1f}s")
                ...
                logger.info(f"  ema load_state_dict done in {_t.time()-_t1:.1f}s")
                logger.info(msg)
                del ema_state_dict
        else:
